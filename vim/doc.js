var Doc = Backbone.Model.extend({
    ...
    newLine: function() {
        ...
        this.get('lines').add(new Line({val:remainder}),{at:cursor.line+1});
            ...
            default:
                switch(key) {
                    ...
                    case '\r':
                        ...
                        break;
                    default:
                        this.get('lines').at(vim.get('cursor').line).get('chars').add({val:key},{at:cursor.char});
    ...
    saveFile: function() {
        ...
        if(this.get('file')) {
            ...
            file.createWriter(function(writer) {
                ...
                writer.write(new Blob[that.toText()], {type: 'text/plain'});
            ...
            file.createWriter(function(writer) {
                log('file.createWriter');
                    ...
                    var blob = new Blob([that.toText()], {type: 'text/plain'});
...
var DocV = Backbone.View.extend({
    render: function() {
        this.model.get('lines').forEach(function(line) {
            var lineV = new LineV({model:line});
            line.set('view',lineV);
