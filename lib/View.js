...
View.prototype.refreshStatusLine = function() {
	...
	if('insert,replace'.indexOf(this.vim.modeName) !== -1) {
		return this.status = ('-- ' + this.vim.modeName.toUpperCase() + ' --').status;
	}
...
View.prototype.getArray = function() {
	...
	var cursorLine = textArray[cursor.line];
	if(!cursorLine) {cursorLine = ''; }
	cursorLine += ' ';
	
	
	textArray[cursor.line] = cursorLine.substring(0,cursor.char) + cursorLine.substring(cursor.char,cursor.char+1).cursor + cursorLine.substring(cursor.char+1);
...
//	var i = this.focus;
	...
	while(linesLeft >= 0) {
		...
		if(typeof textArray[i] !== 'undefined') {
			...
			lines.splice(lines.length-1,0,'~'.idle);
		}
