...
Doc.prototype.find = function(exp,opts) {
	...
	if(backwards) {
		var rest = thisLine.substring(0,this.cursor.char()-1);
	} else {
		var rest = thisLine.substring(this.cursor.char()+1);
	}
	var curIndex = checkString(exp,thisLine,this.cursor.char()+1,backwards);
...
function checkString(exp,str,offset,backwards) {
	...
	var test = exp.exec(str); //TODO fix the corner case here
	...
	if(test) {
		...
		if(backwards) {
			...
			//var ret = str.substring(offset).indexOf(result) + offset;
