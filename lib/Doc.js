...
Doc.prototype.getRange = function(range) {
	...
	if(range[0].line === range[1].line) {
		text = this.line(range[0].line).substring(range[0].char,range[1].char+1);
	} else {
...
//FIXME somehow finding things inside the cursor on backwards searches.
Doc.prototype.find = function(exp,backwards) {
	...
	if(backwards) {
		...
		var rest = this.line().substring(this.cursor.char()+1);
	}
	console.log('char:',this.cursor.char());
	var curIndex = checkString(exp,this.line(),this.cursor.char()+1,backwards);
...
function checkString(exp,str,offset,backwards) {
	...
	var test = exp.exec( (offset ? '|' : '') + str.substring(offset)); //TODO fix the corner case here
	...
	if(test) {
		...
		if(backwards) {
			...
			console.log('getting it via offset',str.substring(offset),result,offset);
			return str.substring(offset).indexOf(result) + offset;
		}
	}
	return -1;
}
