...
Doc.prototype.find = function(exp, opts) {
  ...
  var curIndex = checkString(exp, thisLine, wholeLine ? 0 : (this.cursor.char() + (backwards ? -1 : 1) ), backwards);
...
//	if(!(test[1] || test[2])) console.log('doc.find requires a regular expression with a capture');
