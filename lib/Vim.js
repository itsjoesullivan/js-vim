...
*/
Vim.prototype.buffer = function(k,v) {
	var num = typeof k === 'number';
	if(v) {
		if(num) {
			_numBuffers.splice(num,0,v); //put where
			_numBuffers.splice(10); //only ten.
		} else {
			_buffers[k] = v; //just set if not num TODO: check for '1' case
		}
	} else {
		var buffer = num ? _numBuffers[k] : _buffers[k];
