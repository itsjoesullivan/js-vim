...
module.exports = {
	...
	*/
	...
	'/^(a|i)(w|W|s|S|p|\\]|\\[|\\(|\\)|b|>|<|t|\\{|\\}|"|\'|`)$/': function(keys, vim, match) {
		...
		if(match[2] in boundaryMap) {
			boundaries = boundaryMap[match[2]]
		} else {
			boundaries = [match[2],match[2]];
		}

		var moveIn = false
		if(boundaries[0].substring(0,1) === 'F' && match[1] === 'i') {
			boundaries[0] = 'T' + boundaries[0].substring(1);
			boundaries[1] = 't' + boundaries[1].substring(1);
		} else if(match[1] === 'i') { //fix this
			moveIn = true;
		}
		...
		if(boundaries.length) {
			this.exec('esc');
			this.exec(boundaryMap[match[2]][0])
			if(moveIn) this.exec('l');
			this.exec('v');
			this.exec(boundaryMap[match[2]][1])
