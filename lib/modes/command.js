module.exports = {
	...
	'{operator}{count}{motion}$': function(vim,op,count,motion) {
		var position = vim.curDoc.cursor.position();
	...
	'{count}{operator}{count}{motion}$': function(vim, ct1, operator, ct2, motion) {
	...
	*/
	...
	'^f{char}$': function(vim,character) {  //convert to: f([\w])
		vim.exec('/' + character + '\n');
		vim.last('f','f' + character);
	},
	...
	'^t{char}$': function(vim,character) {  //convert to: f([\w])
		vim.exec('/' + character + '\n');
		vim.exec('h');
		vim.last('f','t' + character);
	},
	...
	'^,$': function(vim) {
		...
		vim.exec(lastOp + last.substring(1));
	},
...
//Insert mode
	...
	'/(\\/|\\?)(.+)\\n/': function(keys,vim,match) {
		...
		var pt = vim.curDoc.find(new RegExp('(' + vim.searchBuffer + ')','g'),{selection: true, backwards: vim.searchMode==='backwards'});
		if(pt) {
...
//		vim.curDoc.cursor.char(position);
	...
	'/([1-9]+[0-9]*)(j|k|w|W|b|B|x|e)/': function(keys,vim,res) {
